...
public class WorkerRunnable implements Runnable {
	...
	//Map runCountsCache = ManagementServices.getBigMap("RunCounts");
	TopicAssignmentBigMap data = (TopicAssignmentBigMap)ManagementServices.getBigMap("TopicAssignment");;
	...
	protected int[][] typeTopicCnt; // indexed by <feature index, topic index>
	protected int[] tokensPerTpc; // indexed by <topic index>
	...
	public int[][] getTypeTopicCounts() {
		return typeTopicCnt;
		//return (int[][])this.runCountsCache.get("Thread_"+this.id+"_tyTpcCnts").getObjectValue();
	}

	public void setTypeTopicCounts(int[][] typeTopicCounts2) {
		//this.runCountsCache.put(new Element("Thread_"+this.id+"_tyTpcCnts",typeTopicCounts2));
		this.typeTopicCnt = typeTopicCounts2;
	}

	public int[] getTokensPerTopic() {
		//return (int[])this.runCountsCache.get("Thread_"+this.id+"_tkpTpc").getObjectValue();
		return tokensPerTpc;
	}

	public void setTokensPerTopic(int[] tokensPerTopic2) {
		this.tokensPerTpc = tokensPerTopic2;
		//this.runCountsCache.put(new Element("Thread_"+this.id+"_tkpTpc",tokensPerTopic2));
	}

	public WorkerRunnable (int id,
						   int numTopics,
						   double[] alpha, double alphaSum,
						   double beta, Randoms random,
						   TopicAssignmentBigMap data,
						   int[][] typeTopicCounts, 
						   int[] tokensPerTopic,
		...
		//this.typeTopicCnt = typeTopicCounts;
		//this.tokensPerTpc = tokensPerTopic;
		this.setTokensPerTopic(tokensPerTopic);
		this.setTypeTopicCounts(typeTopicCounts);
		...
		//System.out.println("WorkerRunnable Thread: " + numTopics + " topics, " + topicBits + " topic bits, " + 
	...
	*/
	...
	public void buildLocalTypeTopicCounts () {
		...
		int[] tokensPerTopic = this.getTokensPerTopic();
		int[][] typeTopicCounts = this.getTypeTopicCounts();
        ...
        for (int doc = startDoc;
			...
			*/
            ...
            BigFeatureSequence tokens = this.data.getTokens(doc);
           
             int[] topics = this.data.getTopicSequence(doc);
		...
		try {
			...
			for (int doc = startDoc;
				 ...
				 doc++) {
				  ...
				  if (doc % 10000 == 0) {
					  //System.out.println("processing doc " + doc + " out of max doc id " +(startDoc + numDocs) +" in thread ==" + this.id);
				  }
				...
				*/
				BigFeatureSequence tokenSequence = this.data.getTokens(doc);
				BigLabelSequence topicSequence = this.data.getTopicSequenceObj(doc);	  
			...
			isFinished = true;
			if(cdl!=null){
				System.out.println("Now counting down in thread " + id); 
				cdl.countDown();
			}
	...
	protected void sampleTopicsForOneDoc (BigFeatureSequence tokenSequence,
										  BigFeatureSequence topicSequence,
		...
		for (int position = 0; position < docLength; position++) {
			...
			if (newTopic == -1) {
				System.out.println("WorkerRunnable sampling error: "+ origSample + " " + sample + " " + smoothingOnlyMass + " " + 
