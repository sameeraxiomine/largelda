...
public class BigFeatureSequence implements Sequence, Serializable, BigAlphabetCarrying
{
	transient BigAlphabet dictionary;
	int[] features;
	int length;
	 ...
	 */
	public BigFeatureSequence (BigAlphabet dict, int[] features)
	{
	...
	public BigFeatureSequence (BigAlphabet dict, int[] features, int len)
	{
		this(dict, len);
		for (int i = 0; i < len; i++)
			add(features[i]);
	...
	public BigFeatureSequence (BigAlphabet dict, int capacity)
	{
		dictionary = dict;
	...
	public BigFeatureSequence (BigAlphabet dict)
	{
		this (dict, 2);
	}
	...
	public BigAlphabet[] getAlphabets() {
		return new BigAlphabet[] {getAlphabet()};
	}
   ...
   sb.append (": ");
	...
	public void add (Object key)
	{
		...
		//System.out.println("Looking up === "+key+"===" + (System.currentTimeMillis()-millis));
		if (fi >= 0)
			add (fi);
	 ...
	 */
 public void prune (double[] counts, BigAlphabet newAlphabet,
                    int cutoff) {
