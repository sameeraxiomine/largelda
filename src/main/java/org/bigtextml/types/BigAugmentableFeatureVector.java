...
public class BigAugmentableFeatureVector extends BigFeatureVector implements Serializable
{
	...
	public BigAugmentableFeatureVector (BigAlphabet dict,
	...
	public BigAugmentableFeatureVector (BigAlphabet dict, double[] values, int capacity) {
		this (dict, null, values, capacity, values.length, true, true, true); }
	public BigAugmentableFeatureVector (BigAlphabet dict, double[] values) {
		this (dict, null, values, values.length, values.length, true, true, true); }

	public BigAugmentableFeatureVector (BigAlphabet dict, int capacity, boolean binary) {
		// yyy
		this (dict, new int[capacity], binary ? null : new double[capacity],
				capacity, 0, false, false, false); }
	public BigAugmentableFeatureVector (BigAlphabet dict, boolean binary) {
		this (dict, 4, binary); }
	public BigAugmentableFeatureVector (BigAlphabet dict) {
		this (dict, false); }
	public BigAugmentableFeatureVector (BigFeatureVector fv) {
		this ((BigAlphabet)fv.dictionary, fv.indices, fv.values,
				fv.indices == null ? fv.values.length : fv.indices.length,
						fv.indices == null ? fv.values.length : fv.indices.length,
								true, false, false);
	}

	public BigAugmentableFeatureVector (BigFeatureSequence fs, boolean binary) {
	 ...
	 */
	public void add (BigFeatureVector fv)
	{
   ...
   double value = fv.valueAtLocation(loc); 
	 ...
	 */
	public void add (BigFeatureVector fv, String prefix)
	{
		BigAlphabet otherDict = fv.getAlphabet ();
		for (int loc = 0; loc < fv.numLocations (); loc++) {
	 ...
	 */
	public void add (BigFeatureVector fv, String prefix, boolean binary)
	{
		if (binary)
			add( fv, prefix);
		else {
	...
	public ConstantMatrix cloneMatrix () {
		return new BigAugmentableFeatureVector ((BigAlphabet)dictionary,
	...
	public ConstantMatrix cloneMatrixZeroed () {
		if (indices == null)
			return new BigAugmentableFeatureVector (dictionary, new double[values.length]);
		else {
			int[] newIndices = new int[indices.length];
			System.arraycopy (indices, 0, newIndices, 0, indices.length);
			return new BigAugmentableFeatureVector (dictionary, newIndices, new double[values.length],
	...
	public BigSparseVector toSparseVector () {
		if (size-1 != maxSortedIndex)
			sortIndices();
		//System.out.println ("AugmentableFeatureVector toSparseVector size="+size);
		return new BigSparseVector (indices, values, size, size, true, false, false);
	}

	public BigFeatureVector toFeatureVector () {
		if (indices != null && size-1 != maxSortedIndex)
			sortIndices();
		return new BigFeatureVector ((BigAlphabet)dictionary,
	...
	public final double dotProduct (BigSparseVector v) {
		if (v instanceof BigAugmentableFeatureVector)
			return dotProduct((BigAugmentableFeatureVector)v);
		if (indices != null && size-1 != maxSortedIndex)
			sortIndices();
		double ret = 0;
		int vl = 0;
	...
	public final double dotProduct (BigAugmentableFeatureVector v) {
		if (indices != null && size-1 != maxSortedIndex)
			sortIndices();
	...
	public void plusEquals (BigAugmentableFeatureVector v, double factor) {
		if (indices != null && size-1 != maxSortedIndex)
			sortIndices();
		if (v.indices != null && v.size-1 != v.maxSortedIndex)
			v.sortIndices();
	...
	public void plusEquals (BigSparseVector v, double factor) {
		if (v instanceof BigAugmentableFeatureVector) {
			plusEquals ((BigAugmentableFeatureVector)v, factor);
			return;
		}
	...
	public void plusEquals (BigSparseVector v) {
		plusEquals (v, 1.0);
	}
