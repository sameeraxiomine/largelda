...
public class BigRankedFeatureVector extends BigFeatureVector
{
	int[] rankOrder;
	private static final int SORTINIT = -1;
	int sortedTo = SORTINIT; /* Extent of latest sort */

	public BigRankedFeatureVector (final BigAlphabet dict,
														 final int[] indices,
														 final double[] values)
	{
	...
	public BigRankedFeatureVector (final BigAlphabet dict, final double[] values)
	{
		super (dict, values);
	}
...
public BigRankedFeatureVector (final BigAlphabet dict, final double[] values, final int begin, final int length)
{
 super (dict, subArray(values, begin, length));
}


public BigRankedFeatureVector (final BigAlphabet dict, final BigDenseVector v)
	{
		this (dict, v.values);
	}

	public BigRankedFeatureVector (final BigAlphabet dict, final BigAugmentableFeatureVector v)
	{
		super (dict, v.indices, v.values, v.size, v.size,
					 true, true, true);
	}

	public BigRankedFeatureVector (final BigAlphabet dict, final BigSparseVector v)
	{
 ...
 for (EntryWithOriginalIndex entry : rankedEntries) {
   ...
   setRankOrder (0);
	...
	public int getMaxValuedIndexIn (final BigFeatureSelection fs)
	{
		if (fs == null) {
   return getMaxValuedIndex();
	...
	public Object getMaxValuedObjectIn (final BigFeatureSelection fs)
	{
   ...
   setRankOrder (0);
	...
	public double getMaxValueIn (final BigFeatureSelection fs)
	{
		if (fs == null) {
   return getMaxValue();
 }
...
public void printLowerK (final PrintWriter out, final int num)
{
 ...
 for (int rank = length-num ; rank < length; rank++) {
   ...
   out.print (obj+":"+val + " ");
	...
	{
		throw new UnsupportedOperationException (BigRankedFeatureVector.class.getName() + " is immutable");
	}


	public interface Factory
	{
		public BigRankedFeatureVector newRankedFeatureVector (BigInstanceList ilist);
	}

	public interface PerLabelFactory
	{
		public BigRankedFeatureVector[] newRankedFeatureVectors (BigInstanceList ilist);
	}
