...
public class BigSparseVector implements ConstantMatrix, Vector, Serializable
{
	/** 
	...
	*/
	...
	public BigSparseVector (int[] indices, double[] values, 
   ...
   throw new IllegalArgumentException
	...
	public BigSparseVector (double[] values, boolean copy)
	{
		this (null, values, values.length, values.length, copy, false, false);
	}

	public BigSparseVector (double[] values) { this (values, true); }

	public BigSparseVector (int size, double fillValue) {
	...
	public BigSparseVector (int[] indices, double[] values, boolean copy)
	{
	...
	public BigSparseVector (int[] indices,
											 boolean copy,
											 boolean checkIndicesSorted,
											 boolean removeDuplicates,
											 boolean binary)
...
/***********************************************************************
	...
	public ConstantMatrix cloneMatrix () {
		if (indices == null)
			return new BigSparseVector (values);
		else
			return new BigSparseVector (indices, values, true, false, false);
	}

	public ConstantMatrix cloneMatrixZeroed () {
		if (indices == null)
			return new BigSparseVector (new double[values.length]);
		else {
			int[] newIndices = new int[indices.length];
			System.arraycopy (indices, 0, newIndices, 0, indices.length);
			return new BigSparseVector (newIndices, new double[values.length], true, false, false);
		}
	}
	 ...
	 */
	public void plusEqualsSparse (BigSparseVector v) {
	 ...
	 *   these are just ignored.
	 */
	public void plusEqualsSparse (BigSparseVector v, double factor)
	{
 ...
 if (indices == null) { densePlusEqualsSparse (v, factor); return; }
	 ...
	 *   these are just ignored.
	 */
	public void timesEqualsSparse (BigSparseVector v) {
	 ...
	 *   these are just ignored.
	 */
	public void timesEqualsSparse (BigSparseVector v, double factor)
	{
 // Special case for dense sparse vector
 if (indices == null) { denseTimesEqualsSparse (v, factor); return; }
	 ...
	 */
	public void timesEqualsSparseZero (BigSparseVector v, double factor)
	{
 // Special case for dense sparse vector
 if (indices == null) { denseTimesEqualsSparse (v, factor); return; }
...
private void densePlusEqualsSparse (BigSparseVector v, double factor)
{
...
private void denseTimesEqualsSparse (BigSparseVector v, double factor)
{
 int maxloc = v.numLocations();
 for (int loc = 0; loc < maxloc; loc++) {
   int idx = v.indexAtLocation (loc);
 ...
 public final void map (Method f) throws IllegalAccessException, Throwable
 {
	...
	public double dotProduct (ConstantMatrix m) {
		if (m instanceof BigSparseVector) return dotProduct ((BigSparseVector)m);
		else if (m instanceof BigDenseVector) return dotProduct ((BigDenseVector)m);
		else throw new IllegalArgumentException ("Unrecognized Matrix type "+m.getClass());
	}

	public double dotProduct (BigDenseVector v) {
	...
	public double extendedDotProduct (BigDenseVector v) {
		double ret = 0;
		if (values == null)
			for (int i = 0; i < indices.length; i++)
				ret += v.value(indices[i]);
	...
	public double dotProduct (BigSparseVector v)
	{
 ...
 //   be more complicated.
 if (v instanceof BigIndexedSparseVector) {
...
private double dotProductInternal (BigSparseVector vShort, BigSparseVector vLong)
{
 double ret = 0;
...
// sets -Inf * 0 = 0, Inf * 0 = 0
	public double extendedDotProduct (BigSparseVector v)
	{
		double ret = 0.0;
		BigSparseVector vShort = null;
		BigSparseVector vLong = null;
	...
	public BigSparseVector vectorAdd(BigSparseVector v, double scale) {
		if(indices != null) { // sparse SparseVector
			...
			return new BigSparseVector(newIndices, newVals, true, true, false);
		}
...
//		return false;
	...
	{
 if (this instanceof BigAugmentableFeatureVector)
   // Be sure to sort/compress our data before we write it
   ((BigAugmentableFeatureVector)this).sortIndices();
